...
int s21_from_int_to_decimal(int src, s21_decimal *dst) {
  *dst = DEC_NULL;
  ...
  if (&src != NULL) {
    if (src < 0) {
      dst->bits[3] = s21_set_bit(dst->bits[3], 31);
      src *= (-1);
    }
    dst->bits[0] = src;
  } else {
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  *dst = 0;
  int flag_err = 0;
  s21_decimal src_t = {0};
  s21_truncate(src, &src_t);
  if (s21_is_greater_or_equal(src_t, DEC_mINT) &&
      s21_is_less_or_equal(src_t, DEC_MINT)) {
    *dst = src_t.bits[0];
    if (s21_get_bit(src_t.bits[3], 31))
      (*dst) *= (-1);
  } else {
    flag_err = 1;
  }
  ...
  if (isnan(src) || isinf(src) || src <= MAX_lim_float * (-1) ||
    ...
    if (src != 0) {
      ...
      for (int i = 23, j = 0; i <= 30; i++, j++) {
        bit = s21_get_bit(fbits_src, i);
        if (bit)
          exp = s21_set_bit(exp, j);
      }
      exp -= 127;
      if (exp > 96) { // 95 //максим число 79228162514264337593543950335.0
        ...
        if (exp < -94 || scale > 28) { //-94 это экспонента мин числа 1e-28
          ...
          while (src > 0) {
            ...
            if (residue) {
              if (count < 32) {
                dst->bits[0] = s21_set_bit(dst->bits[0], count);
              } else if (count >= 32 && count < 64) {
                dst->bits[1] = s21_set_bit(dst->bits[1], count - 32);
              } else if (count >= 64 && count < 96) {
                dst->bits[2] = s21_set_bit(dst->bits[2], count - 64);
              }
            }
            count++;
          }
          s21_set_accur(dst, scale);
          s21_decimal res;
          s21_div(*dst, DEC_1, &res);
          *dst = res;
        }
      }
    }
...
int if_dec_inf_or_nan(s21_decimal *src, int *flag_err) {
  *flag_err = 0;
  ...
  for (int i = 16, j = 0; i < 24; i++, j++) {
    bit = s21_get_bit(src->bits[3], i);
    if (bit)
      exp = s21_set_bit(exp, j);
  }
  if (exp == 255) {
    *flag_err = 1;
    if (bit = s21_get_bit(src->bits[3], 31))
      *flag_err = 2;
    bit = 0;
    for (int i = 0; i < 96 && !bit; i++) {
      if (i < 32) {
        bit = s21_get_bit(src->bits[0], i);
      } else if (i >= 32 && i < 64) {
        bit = s21_get_bit(src->bits[1], i);
      } else if (i >= 64 && i < 96) {
        bit = s21_get_bit(src->bits[2], i);
      }
    }
    if (bit)
      *flag_err = 3;

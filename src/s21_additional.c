...
int s21_get_bit(int value, int index) { return !!(value & (1 << index)); }

int s21_set_bit(int value, int index) { return (value | (1 << index)); }

/* Вставка точности */
void s21_set_accur(s21_decimal *val, int acc) {
  if (acc >= 0 && acc < 29) {
    for (int i = 16; i < 21; i++)
      if (s21_get_bit(val->bits[3], i))
        val->bits[3] ^= (unsigned)pow(2, i);
    for (int i = 0; i < 5; i++)
      if (s21_get_bit(acc, i))
        val->bits[3] = s21_set_bit(val->bits[3], 16 + i);
  }
}
...
void s21_print_bit(s21_decimal value) {
  for (int j = 3; j >= 0; j--) {
    for (int i = 31; i >= 0; i--) {
      printf("%d", s21_get_bit(value.bits[j], i));
    }
    printf("|");
  }
  puts("");
...
int s21_zoom(s21_decimal value) {
  int acc = 0;
  for (int i = 16; i < 21; i++)
    if (s21_get_bit(value.bits[3], i))
      acc = s21_set_bit(acc, i - 16);
  if (acc > 28)
    acc = 28;
  return acc;
}
...
int from_big_to_dec(s21_big_decimal src, s21_decimal *dst) {
  ...
  if (error_code != 2) {
    if (last_num.bits[0] >= 5)
      s21_simple_add(big_dec_one, src, &src);
...
int get_sign_dec(s21_decimal src) { return !!(src.bits[3] & 2147483648); }
...
void equalize_scale(s21_big_decimal *value_1, s21_big_decimal *value_2) {
  ...
  if (scale1 > scale2) {
    for (int i = scale2; i < scale1; i++)
      s21_simple_mul(*value_2, big_dec_ten, value_2);
    set_scale_big_dec(value_2, scale1);
  } else if (scale2 > scale1) {
    for (int i = scale1; i < scale2; i++)
      s21_simple_mul(*value_1, big_dec_ten, value_1);
...
int check_zero(s21_big_decimal *value) {
  ...
  for (int i = 0; i < 6 && zero; i++)
    zero = value->bits[i] == 0;
  if (zero)
    set_sign_big_dec(value, POSITIVE_SIGN);
  return zero;
}
...
int l_shift(s21_decimal *dec, int step) {
  int overflow = 0;
  if (step && s21_is_not_equal(*dec, DEC_NULL)) {
    for (int i = 2; i >= 0; i--) {
      for (int j = 31; j >= 0; j--) {
        if (s21_get_bit(dec->bits[i], j) == 1) {
          if (step + (32 * i) + j > 95) {
            overflow = 1;
            break;
          }
          if (s21_get_bit(dec->bits[i], j)) {
            if (j + step > 63) {
              dec->bits[i + 2] = s21_set_bit(dec->bits[i + 2], j + step - 64);
            } else if (j + step > 31) {
              dec->bits[i + 1] = s21_set_bit(dec->bits[i + 1], j + step - 32);
            } else {
              dec->bits[i] = s21_set_bit(dec->bits[i], j + step);
            }
...
int r_shift(s21_decimal *dec, int step) {
  int error = 0;
  if (step > 0 && s21_is_not_equal(*dec, DEC_NULL)) {
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 32; j++) {
        if (s21_get_bit(dec->bits[i], j)) {
          if (32 * i + j - step < 0) {
            ...
            dec->bits[i - 1] = s21_set_bit(dec->bits[i - 1], 32 + j - step);
          } else if (j - step >= -64) {
            dec->bits[i - 2] = s21_set_bit(dec->bits[i - 2], 32 + j - step);
          }
          dec->bits[i] ^= (unsigned)pow(2, j);
        }
      }

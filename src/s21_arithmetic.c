...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  *result = DEC_NULL;
  s21_big_decimal tmp_1 = {0}, tmp_2 = {0}, tmp_res = {0};
  ...
  if (sign_1 == sign_2) {
    s21_simple_add(tmp_1, tmp_2, &tmp_res);
    set_sign_big_dec(&tmp_res, sign_1);
  } else {
    if (s21_is_greater_or_equal_big(tmp_1, tmp_2)) {
      s21_simple_sub(tmp_1, tmp_2, &tmp_res);
      set_sign_big_dec(&tmp_res, sign_1);
    } else {
      s21_simple_sub(tmp_2, tmp_1, &tmp_res);
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  *result = DEC_NULL;
  ...
  int scale = s21_zoom(value_1) + s21_zoom(value_2);
  ...
  if (!zero) {
    s21_simple_mul(tmp_1, tmp_2, &tmp_res);
    set_scale_big_dec(&tmp_res, scale);
  }
...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  *result = DEC_NULL;
  ...
  int error_status = s21_simple_div(tmp_1, tmp_2, &tmp_res);
  if (error_status == 0) {
...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  *result = DEC_NULL;
  ...
  if (s21_get_bit(value_2.bits[3], 31))
  ...
  if (s21_is_equal(value_2, DEC_NULL)) {
    ...
    *result = value_1;
    ...
    s21_decimal mod = value_2;
    for (int i = 1;; i++) { /* Сдвиг v2 влево к v1 */
      int over = l_shift(&mod, 1);
      if (s21_is_equal(mod, value_1)) { /* v2 равен v1 после сдвигов */
        *result = DEC_NULL;
        break;
      } else if (s21_is_greater(mod, value_1) || over) {
        if (!over)
          r_shift(&mod, 1); /* сдвиг v2 вправо если не было переполнения */
        while (s21_is_greater(value_1, mod)) {
          s21_decimal tmp_sub = {0};
          s21_sub(value_1, mod, &tmp_sub);
          value_1 = tmp_sub;
          while (s21_is_greater(mod, value_1)) {
            if (r_shift(&mod, 1))
              break;
            if (s21_is_less(mod, value_2)) {
              l_shift(&mod, 1);
              break;
            }
          }
        }
        while (s21_is_less(mod, value_1)) {
          s21_decimal tmp_sum = {0};
          error = s21_add(mod, value_2, &tmp_sum);
          if (error > 0)
            break;
          mod = tmp_sum;
        }
        if (s21_is_greater(mod, value_1) && !error) {
          s21_decimal tmp_sum = {0}, tmp_sum2 = {0};
          error = s21_sub(mod, value_2, &tmp_sum);
          if (!error)
            error = s21_sub(value_1, tmp_sum, &tmp_sum2);
          mod = tmp_sum2;
        } else if (s21_is_equal(mod, value_1)) {
          mod = DEC_NULL;
        }
        if (minus == 1)
          s21_negate(mod, &mod);
...
int s21_shift(s21_big_decimal *value, int step) {
  if (step > 0) {
    for (int i = 0; i < step; i++) {
      ...
      if (zero)
        set_bit_big_dec(value, 32);
      if (one)
        set_bit_big_dec(value, 64);
      if (two)
        set_bit_big_dec(value, 96);
      if (three)
        set_bit_big_dec(value, 128);
      if (four)
        set_bit_big_dec(value, 160);
    }
  } else {
    for (int i = 0; i < (-1) * step; i++) {
      ...
      if (zero)
        set_bit_big_dec(value, 31);
      if (one)
        set_bit_big_dec(value, 63);
      if (two)
        set_bit_big_dec(value, 95);
      if (three)
        set_bit_big_dec(value, 127);
      if (four)
        set_bit_big_dec(value, 159);
    }
  }
  return 0;
}
...
int s21_find_nonzero_bit(s21_big_decimal src) {
  ...
  while (result > 0 && !check_bit_big_dec(src, result))
    result--;
...
int s21_simple_mul(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int shift_step = 0;
  reset_big_decimal(result);
  int stop = s21_find_nonzero_bit(value_2);

  for (int idx = 0; idx <= stop; idx++) {
    if (check_bit_big_dec(value_2, idx)) {
      tmp = value_1;
      s21_shift(&tmp, shift_step);
      s21_simple_add(*result, tmp, result);
    }
    shift_step++;
  }
  return error_status;
...
int s21_simple_div(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  if (!s21_is_equal_big(big_dec_zero, value_2)) {
...
s21_big_decimal div_int(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int start_idx = s21_find_nonzero_bit(value_1);
  int shift_step = start_idx - s21_find_nonzero_bit(value_2);

  value_1.bits[6] = 0;
  value_2.bits[6] = 0;

  check_zero(result);
  s21_shift(&value_2, shift_step);

  while (shift_step >= 0) {
    s21_shift(result, 1);
    if (s21_is_greater_or_equal_big(value_1, value_2)) {
      set_bit_big_dec(result, 0);
      s21_simple_sub(value_1, value_2, &value_1);
    }
    s21_shift(&value_2, -1);
...
int div_fractional(s21_big_decimal decimal_reminder, s21_big_decimal value_2,
  ...
  unsigned scale = 0;
  while (!s21_is_equal_big(decimal_reminder, big_dec_zero) && (scale < 29)) {
    s21_simple_mul(big_dec_ten, decimal_reminder, &decimal_reminder);
    s21_simple_mul(*result, big_dec_ten, result);
    scale++;

    if (s21_is_greater_or_equal_big(decimal_reminder, value_2)) {
      decimal_reminder = div_int(decimal_reminder, value_2, &decimal_buffer);
      s21_simple_add(decimal_buffer, *result, result);
    }
  }
...
int trim_fraction(s21_decimal *value) {
  ...
  if (!zero) {
    ...
    while (scale) {
      if (scale == 1)
        s21_simple_mod(big_value, big_dec_ten, &last_num);
...
int s21_simple_mod(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  if (!s21_is_equal_big(decimal_buffer, value_2)) {
